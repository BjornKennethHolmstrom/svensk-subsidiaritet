...
/**
 * Generate PDFs for the AUBI Simulation Report (Swedish and English)
 * 
 * Usage: node scripts/generate-aubi-report-pdfs.js
 * 
 * Output: static/downloads/AUBI-simuleringar-rapport.pdf
 *         static/downloads/AUBI-simulation-report.pdf
 */
...
// --- CONFIGURATION ---
const REPORT_DIR = path.join(__dirname, '../src/lib/content/reports/aubi');
...
const CONFIG = {
    sv: {
        input: path.join(REPORT_DIR, 'aubi-simuleringar-rapport-sv.md'),
        output: path.join(OUTPUT_DIR, 'AUBI-simuleringar-rapport.pdf'),
        title: 'Adaptiv universell basinkomst – simuleringar och analys',
        subtitle: 'Behovsheterogenitet, socialt kapital och kollektiv finansiering',
        langName: 'Svensk Subsidiaritet'
    },
    en: {
        input: path.join(REPORT_DIR, 'aubi-simuleringar-rapport-en.md'),
        output: path.join(OUTPUT_DIR, 'AUBI-simulation-report.pdf'),
        title: 'Adaptive Universal Basic Income – Simulations and Analysis',
        subtitle: 'Need Heterogeneity, Social Capital, and Collective Funding',
...
async function generatePDF(lang) {
    ...
    console.log(`📄 Generating ${lang.toUpperCase()} AUBI report...`);
    ...
    <body>
        ...
        <div class="footer">
            ${lang === 'sv' 
                ? 'Svensk Subsidiaritet – Forskning om adaptiv universell basinkomst' 
                : 'Swedish Subsidiarity – Research on Adaptive Universal Basic Income'}<br/>
            ${SITE_URL}
        </div>
    </body>
    </html>
    ...
    const tempHtmlFile = path.join(OUTPUT_DIR, `temp-render-aubi-${lang}.html`);
...
async function generateAll() {
    console.log('📚 Generating AUBI simulation report PDFs...\n');
