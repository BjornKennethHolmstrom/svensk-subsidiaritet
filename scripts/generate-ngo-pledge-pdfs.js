...
/**
 * Generate PDFs for the NGO Subsidiarity Pledge (Swedish and English)
 * 
 * Usage: node scripts/generate-ngo-pledge-pdfs.js
 * 
 * Output: static/downloads/Lofte-subsidiaritet-for-ngoer.pdf
 *         static/downloads/Subsidiarity-Pledge-for-NGOs.pdf
 */
...
// --- CONFIGURATION ---
const CONTENT_DIR = path.join(__dirname, '../src/routes/verktyg/ngo-lofte');
...
const CONFIG = {
    sv: {
        input: path.join(CONTENT_DIR, 'content.sv.md'),
        output: path.join(OUTPUT_DIR, 'Lofte-subsidiaritet-for-ngoer.pdf'),
        title: 'Löfte om Subsidiaritet för Internationella NGO:er',
        subtitle: 'Från tjänsteleverans till lokalt självbestämmande',
        langName: 'Svensk Subsidiaritet'
    },
    en: {
        input: path.join(CONTENT_DIR, 'content.en.md'),
        output: path.join(OUTPUT_DIR, 'Subsidiarity-Pledge-for-NGOs.pdf'),
        title: 'A Subsidiarity Pledge for International NGOs',
        subtitle: 'From Service Delivery to Local Empowerment',
...
function embedImagesAsBase64(html) {
    ...
    return html.replace(imgRegex, (match, src) => {
        ...
        if (src.startsWith('/')) {
            ...
            filePath = path.join(CONTENT_DIR, src);
        } else {
            filePath = path.join(CONTENT_DIR, src);
        }
...
async function generatePDF(lang) {
    ...
    console.log(`📄 Generating ${lang.toUpperCase()} NGO Pledge...`);
...
async function generateAll() {
    console.log('📚 Generating NGO Pledge PDFs...\n');
